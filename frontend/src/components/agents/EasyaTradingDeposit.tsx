...
import {
  ...
  fetchEasyaUserBalances,
  verifyEasyaDepositWithRetry,
  withdrawEasyaTokens,
  type EasyaDepositVerifyResponse,
  type EasyaTokenBalanceRow,
  type EasyaUserBalances,
} from "@/lib/easyaWalletClient";

const PENDING_DEPOSIT_KEY = "easya_pending_deposit_signature";
...
export function EasyaTradingDeposit({
  ...
  onBalancesChange?: (balances: EasyaUserBalances | null) => void;
  refreshTick?: number;
}) {
  const { connection } = useConnection();
  const { publicKey, sendTransaction, connected } = useWallet();
  const [agentWallet, setAgentWallet] = useState<string | null>(null);
  const [platformFeePct, setPlatformFeePct] = useState("0.1%");
  ...
  const refreshBalances = useCallback(async () => {
    ...
    const data = await fetchEasyaUserBalances(authToken);
    if (data) {
  ...
  async function runDepositVerification(
    ...
    try {
      const verified = await verifyEasyaDepositWithRetry(trimmed, authToken);
      ...
      setSuccess(
        verified.message ??
          (verified.status === "already_recorded"
            ...
            : "Deposit verified and credited to your trading balance.")
      );
  ...
  function applyVerifiedBalances(result: EasyaDepositVerifyResponse) {
  ...
  async function executeWithdraw() {
    ...
    try {
      const result = await withdrawEasyaTokens(
      ...
      setSuccess(`Withdrew ${pendingWithdraw.amount} ${pendingWithdraw.token} to your wallet.`);
    } catch (err) {
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_1fr]">
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Deposit SOL to fund one-time market or limit buys. Platform fee:{" "}
            <span className="text-foreground">{platformFeePct}</span> per successful swap.
            Separate from the DCA agent wallet. After you send funds, your deposit is verified
            automatically and credited to your balance.
          </p>
          ...
          <div className="grid gap-3 sm:grid-cols-[80px_1fr_auto]">
            ...
            <button
              ...
              disabled={busy || !connected || !agentWallet || !amount || !authToken}
            ...
            >
              {busy || verifyBusy ? depositPhase ?? "Processing…" : "Deposit SOL"}
            </button>
          </div>
          ...
          <div className="border-t border-grid pt-4">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              ...
              signature can only be used once and must be a SOL transfer you signed to the
              trading wallet above.
            </p>
          ...
          <div className="border-t border-grid pt-4">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Withdraw unused SOL deposits and tokens acquired from Jupiter buys. Amounts reserved
              for active limit orders cannot be withdrawn.
            </p>
        ...
        <div>
          ...
          {!connected ? (
            ...
            <p className="mt-3 text-sm text-muted-foreground">Sign in with wallet to view balance.</p>
          ) : balances.length === 0 ? (
              ...
              {balances.map((row) => (
                <div key={row.token} className="border border-grid bg-background/60 px-3 py-2.5">
                  ...
                  <div className="mt-1 flex items-center justify-between font-mono text-[10px] uppercase tracking-[0.12em] text-muted-foreground">
                    ...
                    {(row.acquired_from_swaps ?? 0) > 0 && (
                      <span className="text-signal">swaps +{row.acquired_from_swaps}</span>
                    )}
                  </div>
                  {row.mint && (
                  ...
                  <div className="mt-1 grid grid-cols-3 gap-2 font-mono text-[10px] uppercase tracking-[0.12em] text-muted-foreground">
                    ...
                    <span>rsv {row.reserved_for_orders}</span>
